...
rule merge_filtered_subsets:
    ...
    input:
        bcfs = expand(config["results"] + "{{filter_method}}/pass/{{dataset}}.{mode}.bcf",
            mode=["SNP", "indel"]),
        tbis = expand(config["results"] + "{{filter_method}}/pass/{{dataset}}.{mode}.bcf.csi",
            mode=["SNP", "indel"]),
    output:
