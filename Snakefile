...
rule all:
    ...
    Change the path under input to change to change the target files generated by snakemake.
    """
    #input: expand("{output_path}vcf/{sample}.g.vcf.gz", output_path=config["output_path"], sample=SAMPLE_NAMES)
    input:  config["output_path"] + "joint-call/Mmul_8.recalibrated_joint-call.relate"
...
rule estimate_relatedness_lcmlkin:
    ...
    input: #vcf=config["output_path"] + "joint-call/recalibrated_joint-call.vcf.gz"
           vcf=config["lcmlkin"]["vcf"],
           founders=config["lcmlkin"]["founders"]
    output: config["output_path"] + "joint-call/Mmul_8.recalibrated_joint-call.relate"
    threads: 8 # max of 8
    shell: "set -x; lcmlkin \
            -i {input.vcf} \
            -o {output} \
...
rule admixture:
    ...
    input: bed="{name}.bed", bim="{name}.bim", fam="{name}.fam"
    params: ancestral_populations=2
    output: config["output_path"] + "{name}.{params.ancestral_populations}.Q",
            config["output_path"] + "{name}.{params.ancestral_populations}.P"
    threads: 24
    shell: "admixture {input.bed} {params.ancestral_populations} \
            -j{threads}"

rule bed2map:
    input: 
    output:
    shell: "awk '{print $4,$1,$3}'"

rule shapeit4:
    """Estimate haplotypes."""
    input: vcf="", map=config["shapeit4"]["map"]
    output: config["output_path"] + "joint-call/Mmul_8.phased.vcf"
    log: config["output_path"] + "joint-call/phased.log"
    threads: 24
    shell: "shapeit4 \
            --input {input.vcf} \
            --map {input.map} \
